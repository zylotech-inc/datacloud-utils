...
def prepare_search_url(text: str, source: str):
    ...
    domain = website_clean(text)
    if source in ['owler.com', 'aeroleads.com', 'aerolead.com', 'visualvisitor.com', 'visual_visitor']:
        query_params = {"q": f'{source} + "{domain}"'}
    elif source in ['cbinsights.com', 'cb_insights.com']:
...
def send_request_without_proxy(url: str):
    """
    Sends an HTTP GET request to the specified URL without using a proxy.

    Args:
        url (str): The URL to which the request is to be sent.

    Returns:
        BeautifulSoup: A BeautifulSoup object containing the parsed HTML of the response
        if the request is successful (status code 200).
        None: If the request fails (status code not 200).

    Logs:
        - Information about the response status code and URL.
    """
    response = requests.get(url, headers=headers, timeout=120)
    logger.info(f"{response.status_code}:{url}")
    if response.status_code == 200:
        soup = BeautifulSoup(response.text, features=BEAUTIFULLSOUP_PARSER)
        return soup


def send_request(url: str, is_zoominfo_datanyze=False,
                 is_google=False, is_browse_html=False):
    """
    Get the soup element pass the search url and this only use in zoominfo or datanyze
    Args:
        search url (str) : pass the search url
        is_zoominfo_datanyze (bool): Default = False as there are many other sources, 
        this is because zoominfo and datanyze has company-name condition before scraping
        is_browse_html (bool): Default = False This flag will decide whether we want to
        render the html page or just httpresponse

    Returns:
       status_code: status code of the request
       soup : get the soup element
    """
    retries = 0
    soup = ''
    html = ''
    while retries < MAX_RETRY:
        try:
            ...
            data = {
                "url": url,
                "browserHtml": is_browse_html,
                "httpResponseBody": not is_browse_html,
                "javascript": is_browse_html,
            }
            api_response = requests.post('https://api.zyte.com/v1/extract',
                                         json=data, auth=auth, timeout=60)
            ...
            if status_code == 200:
                if "application/json" in content_type:
                    ...
                    if json_response.get("httpResponseBody"):
                        http_response_body: bytes = b64decode(
                            json_response["httpResponseBody"]).decode('utf-8')
                ...
                soup = BeautifulSoup(html, features=BEAUTIFULLSOUP_PARSER)

                if is_zoominfo_datanyze:
                    if soup.find(class_="company-name") and status_code == 200:
                        return html, status_code, soup
                    else:
                        retries += 1
                        logger.info(f"Retrying..{retries}/{MAX_RETRY}")
                        time.sleep(random.randint(10, 30))
                        if retries >= MAX_RETRY:
                            return html, 404, soup

                if is_google:
                    if soup is not None and status_code == 200:
                        return html, status_code, soup
                    else:
                        retries += 1
                        logger.info(f"Retrying..({retries}/{MAX_RETRY})")
                ...
                logger.info(
                    ...
                    "[-] You are using incompatible parameters, such as mixing browserHtml and \
                    httpResponseBody.\n"
                    "[-] There is an issue with your API key: it is missing, malformed, or the \
                    wrong key\n"
                    "[-] Your request body is invalid JSON.\n"
                )
                return html, 400, soup

            elif status_code == 403:
                logger.info("[-] Zyte API sends an HTTP 403 response if you Zyte API account is \
                             suspended.")
                return html, 403, soup
                ...
                logger.info("[-] Server is busy sleeping for 30 seconds")
                retries += 1
                if retries == MAX_RETRY:
                    return html, status_code, soup
                logger.info(f"Retrying..({retries}/{MAX_RETRY})")
            ...
            logger.error("Error occured", exc_info=1)
            retries += 1
            logger.info(f"Max retries reached. Unable to retrieve data.{str(e)}")
            return '', None, ''
    return html, None, soup

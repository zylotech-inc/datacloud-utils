...
class CompanyData(BaseModel):
    ...
    COMPANY_TYPE: Optional[str] = Field(None, description="Type of the company HQ/BR",max_length=2)
    ...
    DELIVERY_DATE: datetime = Field(..., description="Date when the data was delivered")
    ...
    def validate_url(cls, v, info):  # pylint: disable=no-self-argument
        if v in [None,""]:
            return None
...
class LocationData(BaseModel):
    ...
    INFERRED_EMPLOYEES: Optional[Literal['Y', 'N']] = Field(default='N',
...
class ContactData(BaseModel):
    """Contact data model"""
    PRIMARY_DOMAIN: str = Field(None, description="The primary domain associated with the entity")
    ...
    SENIORITY_LEVEL: Optional[str] = Field(None, description="Seniority level of the contact, e.g., Manager, Director")
    ROLE_FUNCTION: Optional[str] = Field(None, description="Functional role of the contact, e.g., Marketing, Sales")
    ...
    MANUAL_CURATION: Optional[Literal['Y', 'N']] = Field('N', description="Tag Y for human-curated, Tag N for automated")
    LINKEDIN_URL: Optional[str] = Field(None, description="LinkedIn profile URL of the contact")
    FACEBOOK_URL: Optional[str] = Field(None, description="Facebook profile URL of the contact")
    TWITTER_URL: Optional[str] = Field(None, description="Twitter profile URL of the contact")
    DELETE_FLAG: Optional[bool] = Field(default=False)
    DELIVERY_DATE: datetime = Field(..., description="Date when the data was delivered")
    SOURCE_CD: str = Field(default='MANUAL', description="Source Code")
    SOURCE_ID: str = Field(..., description="Stage ID with primary_domain and file_name")
    ...
    @field_validator('MANUAL_CURATION', mode='before')
    def convert_bool_to_text(cls, value):  # pylint: disable=no-self-argument
        if isinstance(value, bool):
            return 'Y' if value else 'N'
        # Convert True/False strings to Y/N
...
if __name__ == '__main__':
    ...
    comp_record = {
        ...
        'COMPANY_TYPE': 'BR', 'PRIMARY_INDUSTRY': 'Health, Wellness And Fitness', 'REVENUE': '12323.5',
        'INFERRED_REVENUE_FLAG': '', 'EMPLOYEES': '24.0', 'INFERRED_EMPLOYEES_FLAG': 'False',
        'SPECIALITIES_ARRAY': ["a", "b", "c"],
        'COMPANY_MANUAL_CURATION': 'N', 'ALTERNATE_DOMAIN': '"cisco.com,terminus.com,jio.com,cisco.com"', 'COMPANY_DESCRIPTION': None,
        ...
        'FACEBOOK_URL': 'http://www.facebook.com/grouperoyer',
        'TWITTER_URL': 'http://www.twitter.com/grouperoyer',
        'GICS': "67", 'NAICS': '8989.0', 'SIC': 7011.5, 'DELETE_FLAG': '',
        'DELIVERY_DATE': datetime.today()
    }
    location_record = {
        'SOURCE_ID': '1234567890',
        'PRIMARY_DOMAIN':'terminus.com',
        'LOCATION_ID':'',
        'COUNTRY_CD':'JP',
        'LOCATION_TYPE':'BR',
    ...
    contact_record = {
        ...
        'COUNTRY_CD':'JP',
        'MANUAL_CURATION':'Y',
        'LINKEDIN_URL':'www.linkedin.com/grouperoyer',
        'FACEBOOK_URL':'www.facebook.com/grouperoyer',
        'TWITTER_URL':'www.twitter.com/grouperoyer',
        'DELIVERY_DATE': datetime.today()
    }

    try:
        validated_company_record = comp_validator.create_company_data(comp_record)
        print('########## COMPANY MODEL DUMP ##########')
        print(validated_company_record.model_dump())
        print('########## COMPANY JSON DUMP ##########')
        print(validated_company_record.model_dump_json())
    except ValidationError as verror:
        print('Unable to validate the fields:', str(verror))
    except ValueError as e:
        print(e)
    try:
        validated_location_record = LocationData(**location_record)
        print('########## LOCATION MODEL DUMP ##########')
        print(validated_location_record.model_dump())
        print('########## LOCATION JSON DUMP ##########')
        print(validated_location_record.model_dump_json())
    except ValidationError as verror:
        print('Unable to validate the fields:', str(verror))
    except ValueError as e:
        print(e)
    try:
        validated_contact_record = ContactData(**contact_record)
        print('########## CONTACT MODEL DUMP ##########')
        print(validated_contact_record.model_dump())
        print('########## CONTACT JSON DUMP ##########')
        print(validated_contact_record.model_dump_json())
    except ValidationError as verror:
        print('Unable to validate the fields:', str(verror))
    except ValueError as e:
        print(e)